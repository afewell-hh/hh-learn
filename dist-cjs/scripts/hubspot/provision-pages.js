...
async function findPageBySlug(slug) {
    ...
    try {
        ...
        const searchRes = await httpRetry(async () => {
    ...
    try {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            const listRes = await httpRetry(async () => {
            ...
            const results = (listRes?.results ?? listRes ?? []);
...
async function findPagesBySlugPrefix(slug) {
    const results = [];
    let after = undefined;
    const token = (0, get_hubspot_token_js_1.getHubSpotToken)();
...
async function findPrimaryPageBySlug(slug) {
    try {
        ...
        if (exact)
            return exact;
        pages.sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
        return pages[0];
    }
    catch (err) {
...
async function validateTemplateExists(templatePath) {
    let token;
    try {
        token = (0, get_hubspot_token_js_1.getHubSpotToken)();
    }
    catch (err) {
    ...
    try {
        ...
        const response = await retryWithBackoff(async () => {
            const res = await fetch(`${base}/cms/v3/source-code/draft/metadata/${encodeURIComponent(templatePath)}`, {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Accept': 'application/json'
...
async function checkForMisnamedTemplates(dryRun = false) {
...
async function createOrUpdatePage(config, tableId, dryRun = false, publish = false) {
    ...
    try {
        ...
        if (existingPage) {
            ...
            page = await retryWithBackoff(() => hubspot.cms.pages.sitePagesApi.update(String(existingPage.id), pagePayload));
            ...
            page = await retryWithBackoff(() => hubspot.cms.pages.sitePagesApi.create(pagePayload));
        ...
        if (publish && page.state !== 'PUBLISHED') {
            ...
            try {
                ...
                const api = hubspot.cms.pages.sitePagesApi;
...
async function provisionPages(dryRun = false, publish = false) {
    ...
    for (const config of pageConfigs) {
        try {
            ...
            const result = await createOrUpdatePage(config, tableId || '<table-id-required>', dryRun, publish);
            if (result) {
                results.push(result);
            }
