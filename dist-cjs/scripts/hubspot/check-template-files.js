...
async function checkTemplate(path) {
    try {
        const response = await fetch(`https://api.hubapi.com/cms/v3/source-code/draft/metadata/${encodeURIComponent(path)}`, {
            headers: {
...
async function checkAllPages() {
    ...
    for (const page of pages) {
        try {
            const response = await fetch(`https://api.hubapi.com/cms/v3/pages/site-pages/${page.id}`, {
                headers: {
                    'Authorization': `Bearer ${TOKEN}`
                }
            });
