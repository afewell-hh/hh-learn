...
const HUBSPOT_TOKEN = (0, get_hubspot_token_js_1.getHubSpotToken)();
...
function collectFiles(localDir, remoteDir) {
    const files = [];
    const entries = (0, fs_1.readdirSync)(localDir);
    for (const entry of entries) {
        const localPath = (0, path_1.join)(localDir, entry);
        const remotePath = `${remoteDir}/${entry}`;
        const stat = (0, fs_1.statSync)(localPath);
        if (stat.isDirectory()) {
...
async function uploadFile(file, dryRun = false) {
    ...
    try {
        ...
        const response = await retryWithBackoff(async () => {
            ...
            const res = await fetch(`https://api.hubapi.com/cms/v3/source-code/draft/content/${encodeURIComponent(file.remotePath)}`, {
                method: 'PUT',
                headers: {
...
async function uploadTemplates(dryRun = false) {
    ...
    const filtered = files.filter((f) => override || ALLOWED_REMOTE_PREFIXES.some((p) => f.remotePath.startsWith(p)) || f.remotePath === ALLOWED_SINGLE_FILE);
    ...
    console.log(`🔐 Using HubSpot token: ${(0, get_hubspot_token_js_1.maskToken)(HUBSPOT_TOKEN)}`);
    // Upload each file
    let successCount = 0;
    let failCount = 0;
