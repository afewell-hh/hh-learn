...
- [Automation Helpers](#part-55-authentication-helpers-for-automation-) - Test-only JWT context 🔐
...
## Part 5.5: Authentication Helpers for Automation 🔐

### Production Baseline – HubSpot Membership (Issues #270/#272/#274)

- CTAs on public course/pathway pages render server-side using `request_contact.is_logged_in` and `personalization_token('contact.email','')`.
- Anonymous visitors see a login link built with `/_hcms/mem/login?redirect_url=<current page>`.
- `clean-x-hedgehog-templates/assets/js/auth-context.js` hydrates `window.hhIdentity` from HubL data attributes (no bootstrapper, no `window.hhIdentity.login`).
- `clean-x-hedgehog-templates/assets/js/enrollment.js` only upgrades authenticated CTAs and handles CRM enrollment checks.
- Verification artifacts: `verification-output/issue-274/` (anonymous/authenticated HTML captures, implementation notes, manual logs).

### Automation Helper – JWT `/auth/login` (Test-only)

- **Purpose:** Speed up Playwright or API automation when HubSpot’s membership form is brittle or when tests must bypass UI latency.
- **Endpoint:** `POST /auth/login` serviced by `src/api/lambda/index.ts` (`login` handler).
- **Helper:** `tests/helpers/auth.ts` exposes `membershipLogin(page)` (preferred) and `jwtLogin(page)` (fallback). Both reset state after use.
- **Secret:** `JWT_SECRET` stored in AWS SSM (`/hhl/jwt-secret`) and GitHub Actions secrets; never exposed to browser code.
- **Guardrails:**
  - No production template references `window.hhIdentity.login`.
  - Docs (`docs/auth-and-progress.md`, `HUBSPOT-AUTH-QUICK-SUMMARY.md`) label JWT as automation-only.
  - If JWT helpers change, update verification outputs and this section immediately.

### Which Path To Use

| Scenario | Authentication Path | Notes |
|----------|--------------------|-------|
| Manual QA / customer flow | HubSpot membership (`/_hcms/mem/login`) | Validates the real user experience |
| Playwright happy-path tests | `await membershipLogin(page)` | Uses stored credentials (`HUBSPOT_TEST_EMAIL/PASSWORD`) |
| Playwright fixtures / CI flakiness | `await jwtLogin(page)` | Requires `JWT_SECRET`; call sparingly |
| API smoke tests | HubSpot Project Access Token | JWT not required |

### Test Commands

```bash
# Membership flow regression (CTA + enrollment)
npx playwright test tests/e2e/enrollment-cta.spec.ts

# JWT helper coverage (automation-only)
npx playwright test tests/e2e/native-login-flow.spec.ts --grep "jwt"

# API smoke in CI (automation context)
curl -X POST "$API_BASE/auth/login" \
  -H "Content-Type: application/json" \
  -d '{"email": "'"$HUBSPOT_TEST_EMAIL"'"}'
```

### Operational Reminders

- Do **not** reintroduce `window.hhIdentity.login` or localStorage bootstrap logic into production code.
- When debugging auth issues, start with Issues #270/#272/#274 and the evidence in `verification-output/issue-274/`.
- Keep JWT helpers limited to CI/automation. If you need broader access, open an issue with justification and updated threat modeling.
- Update `docs/auth-and-progress.md` whenever helper behavior changes so new contributors stay aligned.

> **If you are troubleshooting authentication for real users, the answer is always “use HubSpot membership.” JWT is only here to keep automated coverage unblocked.**
