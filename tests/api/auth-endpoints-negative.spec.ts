...
test.describe('Auth Endpoints - Negative Tests (Issue #303)', () => {
  ...
  test.describe('GET /auth/login - Error Cases', () => {
    ...
    test('should handle extremely long redirect_url gracefully', async ({ request }) => {
      ...
      const response = await request.get(`${AUTH_BASE_URL}/auth/login?redirect_url=${encodeURIComponent(longUrl)}`);
    ...
    test('should sanitize redirect_url to prevent open redirect', async ({ request }) => {
      ...
      const response = await request.get(`${AUTH_BASE_URL}/auth/login?redirect_url=${encodeURIComponent(evilUrl)}`, {
        maxRedirects: 0
      });
    ...
    test('should reject javascript: protocol in redirect_url', async ({ request }) => {
      ...
      const response = await request.get(`${AUTH_BASE_URL}/auth/login?redirect_url=${encodeURIComponent(xssUrl)}`);
    ...
    test('should handle malformed redirect_url gracefully', async ({ request }) => {
      ...
      const response = await request.get(`${AUTH_BASE_URL}/auth/login?redirect_url=${malformedUrl}`);
    ...
    test('should generate unique code_challenge on each request', async ({ page }) => {
      ...
      await page.goto(`${AUTH_BASE_URL}/auth/login?redirect_url=/learn`);
  ...
  test.describe('GET /auth/callback - Error Cases', () => {
    ...
    test('should return 400 when code parameter is missing', async ({ request }) => {
      const response = await request.get(`${AUTH_BASE_URL}/auth/callback?state=validstate123`);
    ...
    test('should return 400 when state parameter is missing', async ({ request }) => {
      const response = await request.get(`${AUTH_BASE_URL}/auth/callback?code=authcode123`);
    ...
    test('should reject callback with invalid authorization code', async ({ request }) => {
      const response = await request.get(`${AUTH_BASE_URL}/auth/callback?code=invalid_auth_code&state=valid_state`);
    ...
    test('should validate PKCE code_verifier matches original code_challenge', async ({ request }) => {
      ...
      const response = await request.get(`${AUTH_BASE_URL}/auth/callback?code=auth_code&state=state_with_wrong_verifier`);
    ...
    test('should reject state parameter with invalid signature/format', async ({ request }) => {
      const response = await request.get(`${AUTH_BASE_URL}/auth/callback?code=authcode&state=obviously_tampered_state`);
    ...
    test('should handle Cognito error parameter', async ({ request }) => {
      ...
      const response = await request.get(`${AUTH_BASE_URL}/auth/callback?error=access_denied&error_description=User+denied+access`);
    ...
    test('should prevent CSRF by validating state', async ({ request }) => {
      ...
      const response = await request.get(`${AUTH_BASE_URL}/auth/callback?code=code_from_different_session&state=state_from_another_session`);
    ...
    test('should handle Cognito token exchange failure', async ({ request }) => {
      ...
      const response = await request.get(`${AUTH_BASE_URL}/auth/callback?code=expired_or_used_code&state=valid_state`);
    ...
    test('should reject callback with SQL injection attempt in parameters', async ({ request }) => {
      ...
      const response = await request.get(`${AUTH_BASE_URL}/auth/callback?code=${encodeURIComponent(sqlInjection)}&state=valid`);
  ...
  test.describe('POST /auth/logout - Error Cases', () => {
    ...
    test('should handle logout when user is not logged in', async ({ request }) => {
      // No cookies set
      const response = await request.post(`${AUTH_BASE_URL}/auth/logout`);
    ...
    test('should clear cookies even if Cognito logout fails', async ({ page, context }) => {
      ...
      await page.request.post(`${AUTH_BASE_URL}/auth/logout`);
    ...
    test('should handle logout with invalid/expired token gracefully', async ({ page, context }) => {
      ...
      const response = await page.request.post(`${AUTH_BASE_URL}/auth/logout`);
  ...
  test.describe('Rate Limiting', () => {
    ...
    test('should rate limit excessive login requests', async ({ request }) => {
      ...
      const requests = [];
      for (let i = 0; i < 20; i++) {
        requests.push(request.get(`${AUTH_BASE_URL}/auth/login`));
      }
    ...
    test('rate limit error should include Retry-After header', async ({ request }) => {
      // Trigger rate limit
      for (let i = 0; i < 10; i++) {
        await request.get(`${AUTH_BASE_URL}/auth/login`);
      }

      const response = await request.get(`${AUTH_BASE_URL}/auth/login`);
  ...
  test.describe('Security Headers', () => {
    ...
    test('all auth endpoints should include security headers', async ({ request }) => {
      ...
      for (const endpoint of endpoints) {
        const response = await request.get(`${AUTH_BASE_URL}${endpoint}`);
  ...
  test.describe('Concurrent Requests', () => {
    ...
    test('should handle concurrent /auth/me requests correctly', async ({ page, context }) => {
      ...
      const requests = Array(10).fill(null).map(() =>
        page.request.get(`${AUTH_BASE_URL}/auth/me`)
      );
